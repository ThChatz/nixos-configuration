...
** TODO declarative users + passwords
...
distinct '@persistent/<username>' and
'@persistent/cache/<username>' subvolumes,

snapshot only @persistent/<username>
@persistent/cache is only for persistent files that should not be in backup (gpg db, browser data(?))

maybe not per username caches since we don't snapshot these
